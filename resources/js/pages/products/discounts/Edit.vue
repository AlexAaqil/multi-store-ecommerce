...
const form = useForm({
    ...
    category_ids:     props.discount.categories?.map(c => c.id) || [],
    product_ids:      props.discount.products?.map(p => p.id) || [],
    min_order_amount: props.discount.min_order_amount?.toString() || '',
    min_quantity:     props.discount.min_quantity?.toString() || '',
...
<template>
    ...
    <div class="py-8 px-4">
        <div class="mb-6">
            <h1 class="text-2xl font-serif font-semibold">Edit Discount</h1>
            <p class="text-sm text-gray-500 mt-1">Update discount for {{ shop.name }}</p>
        </div>
